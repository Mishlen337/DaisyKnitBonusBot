...
class TokenRepository:
    ...
    async def get_all(self, token="", vacant: Optional[bool] = "") -> List[Optional[dict]]:
    ...
    async def get_one(self, token="", vacant: Optional[bool] = "") -> Optional[dict]:
        ...
        tokens = await self.get_all(token=token, vacant=vacant)

        if tokens and tokens[0]:
            return tokens[0]
        return None

    async def delete(self, token="", vacant: Optional[bool] = "") -> None:
